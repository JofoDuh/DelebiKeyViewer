...
namespace DelebiKeyViewer
{
    public static class Main
    {
        ...
        public static scrDelebiKeyViewer KeyViewer;
        ...
        private static bool OnToggle(UnityModManager.ModEntry modEntry, bool value)
        {
            ...
            if (value)
            {
                ...
                DelebiBundleManager bundleManager = DelebiBundleManager.Instance = new DelebiBundleManager();
                GameObject gameObject = UnityEngine.Object.Instantiate(bundleManager.KeyViewerObject);
                UnityEngine.Object.DontDestroyOnLoad(gameObject);
                KeyViewer = gameObject.GetComponent<scrDelebiKeyViewer>();
        ...
        {
            GUILayout.Label($"{setting.GetLocalized("dkv.settingslabel")}");
            ...
            float newLocationX = NamedSlider($"{setting.GetLocalized("dkv.posx")}:", setting.LocationX, 0, 1, 250);
            if (newLocationX != setting.LocationX)
            {
                setting.LocationX = newLocationX;
                UpdateLocation();
            }
            float newLocationY = NamedSlider($"{setting.GetLocalized("dkv.posy")}:", setting.LocationY, 0, 1, 250);
            if (newLocationY != setting.LocationY)
            {
                setting.LocationY = newLocationY;
                UpdateLocation();
            }
            bool newFlipHorizontal = GUILayout.Toggle(setting.FlipHorizontal, $"{setting.GetLocalized("Dkv.fliphorizontal")}:");
            ...
            // Top Row
            GUILayout.Space(18f);
            GUILayout.Label(setting.GetLocalized("dkv.toprow"));
            ...
            // Bottom Row
            GUILayout.Label(setting.GetLocalized("dkv.bottomrow"));
            GUILayout.BeginHorizontal();
            for (int i = 8; i < 10; i++) CreateButton(KeyInputManager.HandLocation[i]);
            GUILayout.FlexibleSpace();
            GUILayout.EndHorizontal();

            if (SelectedKey == -1 || KeyPressed == null) return;
            GUILayout.Label($"<b>{setting.GetLocalized("dkv.pressakey")}</b>");
            if (Input.anyKeyDown)
            {
        ...
        private static void Winking()
        {
            try
            {
                while (IsEnabled)
                {
                    ...
                    } while (KeyViewer.isSmashing || KeyViewer.gameResult);
                    KeyViewer.winkOn = true;
                    KeyViewer.Delebi.sprite = DelebiBundleManager.Instance.DelebiIdleWink;
                    ...
                    KeyViewer.Delebi.sprite = DelebiBundleManager.Instance.DelebiIdle;
                    KeyViewer.DelebiSmash.enable = 0;
                    KeyViewer.DelebiClear.enable = 0;
                }
            }
...
namespace DelebiKeyViewer
{
    public static class DelebiKeyViewerInfo
    {
        public const string PLUGIN_GUID = "BepInEx5.DelebiKeyViewer";
        public const string PLUGIN_NAME = "DelebiKeyViewer";
    ...
    [BepInPlugin(DelebiKeyViewerInfo.PLUGIN_GUID, DelebiKeyViewerInfo.PLUGIN_NAME, DelebiKeyViewerInfo.PLUGIN_VERSION)]
    public class Main : BaseUnityPlugin
    {

        public static Main Instance;
        public static ManualLogSource Logger;
        public static string ModPath => Path.Combine(Paths.PluginPath, "DelebiKeyViewer");
        public static Setting setting;
        public static scrDelebiKeyViewer KeyViewer;
        public static DelebiBundleManager BundleManager;
        public static bool IsEnabled = false;
        ...
        private void Awake()
        {
            ...
            setting = new Setting(); // You may need to pass the correct path
                                     // Init bundle manager
            BundleManager = DelebiBundleManager.Instance = new DelebiBundleManager();

            // Load viewer
            GameObject obj = Instantiate(BundleManager.KeyViewerObject);
            DontDestroyOnLoad(obj);
            KeyViewer = obj.GetComponent<scrDelebiKeyViewer>();
            ...
            harmony = new Harmony(DelebiKeyViewerInfo.PLUGIN_GUID);
            harmony.PatchAll(Assembly.GetExecutingAssembly());
        ...
        private void Winking()
        {
            try
            {
                while (true)
                {
                    ...
                    if (KeyViewer.isSmashing || KeyViewer.gameResult)
                        continue;

                    KeyViewer.winkOn = true;
                    KeyViewer.Delebi.sprite = BundleManager.DelebiIdleWink;
                    KeyViewer.DelebiSmash.enable = 0;
                    KeyViewer.DelebiClear.enable = 0;
                    ...
                    KeyViewer.winkOn = false;
                    KeyViewer.Delebi.sprite = BundleManager.DelebiIdle;
                }
            }
            catch (ThreadAbortException)
            {
        ...
        private void OnDestroy()
        {
            ...
            harmony?.UnpatchAll(DelebiKeyViewerInfo.PLUGIN_GUID);
        }
    }
}
#endif